...
int func(int i,int buy,vector<int>& prices,vector<vector<int>> &dp){
        ...
        if(buy) return dp[i][buy] = max( (-prices[i] + func(i+1,0,prices,dp)),func(i+1,1,prices,dp));
        else return dp[i][buy] = max( (prices[i] + func(i+1,1,prices,dp)), func(i+1,0,prices,dp));
